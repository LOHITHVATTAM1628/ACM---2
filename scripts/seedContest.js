...
// IMPORTANT:
// Service role key must NEVER use the VITE_ prefix.
// VITE_ variables can be exposed to the frontend.
...
if (!supabaseKey) {
  ...
  console.error('⚠️ Add your Supabase service role key to .env');
  process.exit(1);
}
...
console.log('🔐 Running with Service Role privileges\n');

// =====================================================
// 21-DAY SQL CONTEST DATA
// =====================================================
...
Retrieve the id, full_name, and gpa of all active students
with a GPA of 3.5 or higher.

Sort the results by gpa in descending order,
and limit the output to the top 5 students.
`,
...
who earn a salary strictly greater than 80,000.
...
`,
...

  {
    ...
    description: `
Search the product inventory for items whose name contains 'Pro'
and have a unit price between 20.00 and 150.00 inclusive.

Return product_id, product_name, and price
...
`,
...

  {
    ...
    description: `
Extract unique department disciplines offering courses
with 3 or more credits.

Return the distinct department as offering_dept
...
`,
    ...
    solution_query: `
SELECT DISTINCT department AS offering_dept
FROM courses
WHERE credits >= 3
...
`,
...

  {
    ...
    description: `
Find all emergency patients admitted in rooms
ER-1, ER-2, ER-3 who have an assigned attending physician.

Return patient_id, patient_name, room_number,
and attending_physician ordered by patient_id ascending.
`,
...
`,
    ...
    solution_query: `
SELECT patient_id, patient_name, room_number, attending_physician
FROM patients
...
- average sale as avg_sale

Round avg_sale to 2 decimal places.
...
Find the total number of orders as order_count
and total money spent as total_spent for each customer_id.
...
`,
...

  {
    ...
    description: `
Identify high-compensation departments with substantial teams.

Calculate:
- average salary as avg_salary
- employee headcount as headcount

Filter departments with:
- more than 2 employees
- average salary exceeding 75,000
...
`,
...

  {
    ...
    description: `
Classify warehouse items into stock tiers:

- quantity >= 100 -> Optimal
- quantity BETWEEN 30 AND 99 -> Moderate
- Else -> Low Stock

Return item_id, item_name, quantity,
and computed status as stock_status.
...
`,
...

  {
    ...
    description: `
Analyze web traffic logs to find viral articles.

For each article_slug, find:
- total views as total_views
- number of unique visitors as unique_visitors

Only return articles with 3 or more unique visitors.
...
`,
...

  {
    ...
    description: `
Match published books with author details.
...
Perform a LEFT JOIN between clients and client_orders
on client_id.

Return client_id, client_name, signup_date
for clients with no matching orders.

Sort by client_id ASC.
`,
...
`,
...

  {
    ...
    description: `
Build an academic roster combining students,
enrollments, and courses across 3 tables.
...
Only enrollments with grade 'A'.

Sort by student_name ASC,
then course_code ASC.
`,
...
`,
...

  {
    ...
    description: `
Find employees whose salary is strictly greater
...
`,
...

  {
    ...
    description: `
Identify all projects that have at least one assigned engineer
with a 'Lead' title.
...
`,
...

  {
    ...
    description: `
For each bank account, find the single most recent transaction.

Use ROW_NUMBER() partitioned by account_id
and ordered by txn_date DESC.
...
Only the latest transaction per account.
Sort by account_id ASC.
`,
...
`,
...

  {
    ...
    description: `
Rank chapter participants based on total contest score
using DENSE_RANK().

Tied scores must receive identical rank numbers
with no gaps in subsequent rankings.
...
Order by rank ASC, then user_name ASC.
`,
...
`,
...

  {
    ...
    description: `
Calculate month-over-month revenue differentials
using the LAG() window function.

For each month, return:
- month_num
- revenue
- previous month's revenue as prev_month_revenue
- growth

For month 1, previous revenue should be 0.

Sort by month_num ASC.
`,
...
`,
...

  {
    ...
    description: `
Compute a cumulative running total of store revenue day-by-day
using an unbounded preceding window frame.
...
`,
...

  {
    ...
    description: `
Traverse an organizational hierarchy tree
using a recursive CTE.

Start at the CEO where manager_id IS NULL
at level = 1.

Recursively traverse to find each employee's depth.
...
Order by level ASC, then emp_id ASC.
`,
...
`,
...

  {
    ...
    description: `
Congratulations on reaching Day 21!

Construct a multi-metric customer intelligence query using CTEs.

1. Calculate each customer's lifetime spend as lifetime_spend
   and order count as total_orders.

2. Assign a customer tier:
   - VIP if lifetime_spend >= 1000
   - Standard otherwise

3. Use DENSE_RANK() on lifetime_spend DESC
   to generate spend_rank.
...
async function seedContest() {
  ...
  console.log('🚀 Initializing 21-Day SQL Contest Database...');
  console.log(`📡 Connecting to: ${supabaseUrl}`);
  console.log('🔐 Client Mode: Service Role\n');
  ...
  for (const challenge of challenges) {
    ...
    try {
      ...
      if (error) {
        ...
        console.error(
          `❌ Day ${String(challenge.day_number).padStart(2, '0')} - ${challenge.title}`
        );
      ...
      console.error(
        `❌ Day ${challenge.day_number} Exception:`,
        error.message
      );
  ...
  console.log('\n');
  console.log('==========================================');
  console.log('🏁 SQL CONTEST SEEDING COMPLETED');
  console.log('==========================================');
  ...
  if (failCount === 0) {
    ...
    console.log(
      '\n🎉 All 21 SQL challenges seeded successfully!'
    );

  } else {

    console.log(
      '\n⚠️ Some challenges failed.'
    );
...
seedContest()
  ...
  .catch((error) => {
    ...
    console.error(
      '\n🔥 Fatal error while running seed script:'
    );
